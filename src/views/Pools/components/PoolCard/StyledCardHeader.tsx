...
const StyledCardHeader: React.FC<{
  ...
  const isCakePool = earningTokenSymbol === 'GLENTY' && stakingTokenSymbol === 'GLENTY'
  ...
  const getSubHeading = () => {
    ...
    if (isCakePool) {
      return t('Earn GLENTY, stake GLENTY')
    }
  ...
  return (
    <Wrapper isPromotedPool={isPromotedPool} isFinished={isFinished} background={background}>
      <Flex alignItems="center" justifyContent="space-between">
        ...
        <Image src={`/images/pools/${poolImageSrc}`} alt={earningTokenSymbol} width={94} height={64} />
      </Flex>
    </Wrapper>
  )
}
